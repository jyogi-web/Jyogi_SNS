...
| TTL戦略   | `todos` / `make_stamp` は24時間後に削除（アプリ層 or バッチで管理）                 |
...
| created_at | TIMESTAMPTZ | DEFAULT NOW()                | 投稿日時（TTL判定基準）          |
...
| created_at     | TIMESTAMPTZ | DEFAULT NOW() | 作成日時（TTL基準） |
...
| weather     | TEXT        | NOT NULL CHECK IN ('sunny','cloudy','rainy','snowy','stormy')        | 天気種別                             |
...
| 天気いいね `weather.likes` | likesテーブルとの分離管理    | 統合管理テーブルへの移行を検討                   |
